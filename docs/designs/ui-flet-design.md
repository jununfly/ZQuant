...
> 完整领域词汇见根目录 `ZJ-CONTEXT.md`（统一领域语言）。

| 术语 | 含义 |
|------|------|
| Flet | 本系统选定的界面框架（Flutter 渲染，纯 Python） |
| API 层 | 新增薄 FastAPI 适配层，统一暴露 core 能力 |
| 概览/扫描/仓位/回测 | 四个一级页面（概览页技术层对应 `status`） |
| 活筹 / 盘态 | 大盘择时数据及其多空判定（多头/震荡/空头） |
| 主线/支线/答应 | 仓位三层（答应 = 防御层，zettaranc 原话） |
| 仓位 vs 持仓 | 仓位=资金分配规划（目标）；持仓=实际持有的股票市值（事实） |
| 仓位双口径 | 层占比（主线50-70%）相对总仓位；层金额/展示比例（主线48%）相对总资产 |
| B1/B2/B3a/B3b、S1/S2/S3、DD | 买卖点与风控信号（B3 为系列名，实际触发 B3a/B3b） |
