...
> 数据文件: `roadmap.json` | 最后更新: 2026-08-19 01:30:00
...
├── [x][X+] 1-11. M5: 回测引擎
├── [x][X+] 1-12. M6: TUI完整面板
└── [x][X+] 1-13. M7: 多端界面(Reflex)
    ├── [x][X+] 1-13-1. P0: 薄FastAPI API层
    ├── [x][X+] 1-13-2. P1: Flet骨架(四页导航)
    ├── [x][X+] 1-13-3. P2: 可视化落地(flet-charts)
    ├── [x][X+] 1-13-4. P3: 多端打包+agent验证
    ├── [x][X+] 1-13-5. Reflex 依赖接入+骨架初始化
    ├── [x][X+] 1-13-6. 四页界面移植(概览/扫描/仓位/回测)
    ├── [x][X+] 1-13-7. 图表落地(Reflex组件)
    └── [x][X+] 1-13-8. agent-browser 验证+响应式适配
└── [~][X+] 1-14. M8: 持仓顾问(advisor)
    ├── [x][X+] 1-14-1. a原型: 核心链路(活筹单维+A股S/DD)
    ├── [ ][X+] 1-14-2. 港股/ETF数据源(Tushare)
    ├── [ ][X+] 1-14-3. 多维信号灯(量能/指数/主线/情绪)
    └── [ ][X+] 1-14-4. CLI zquant advise + Reflex第五页

### 当前施工：1-14. M8: 持仓顾问(advisor)

M8 已立顶并开工：1-14-1 原型(活筹单维 + A股真实 S/DD 信号)已完成，用 7 只示例持仓跑通 ZJ-Advisor 式「诊断 + 极简执行单」(总仓 54.81% → 目标 20.74%，红线 30%)；1-14-2/3/4 待补(港股 ETF 数据源 / 多维信号灯 / CLI+Web 集成)。
...
**M8 决策：**
- Q: 实现取向? → 纯程序化(core算结构化诊断+执行单, Reflex直接展示); 不用LLM (用户选择 a 原型先行)
- Q: 信号灯维度? → 多维5维(活筹/量能/指数/主线强度/情绪); 原型先活筹单维 (待补量能/指数等数据源)
- Q: 数据范围? → 含港股+ETF; 接Tushare覆盖H09988/H00700/510300 (需用户提供token)
